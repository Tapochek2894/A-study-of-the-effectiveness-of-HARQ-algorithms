...
int main(int argc, char** argv) {
  ...
  for (std::size_t idx = 0; idx < snr_values.size(); ++idx) {
    ...
    // LLR с учётом замираний: L = 2·μ·r / σ².
    std::vector<double> coded_llr =
        channel.ComputeLlr(coded_received, channel.last_fading_amplitudes());

    std::vector<uint8_t> decoded_data;
    decoded_data.reserve(info_bits);
    for (std::size_t i = 0;
         i + static_cast<std::size_t>(n) <= coded_llr.size();
         i += static_cast<std::size_t>(n)) {
      std::vector<double> cw(
          coded_llr.begin() + static_cast<std::ptrdiff_t>(i),
          coded_llr.begin() + static_cast<std::ptrdiff_t>(i + n));
