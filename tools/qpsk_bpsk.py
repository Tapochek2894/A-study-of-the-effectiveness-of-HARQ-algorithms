...
def main():
    ...
    with args.csv_path.open() as f:
        ...
        for row in reader:
            ...
            # Try different column names (with and without _uncoded)
            if "bpsk_ber_uncoded" in fieldnames:
                bpsk_ber.append(float(row["bpsk_ber_uncoded"]))
            elif "bpsk_ber" in fieldnames:
                bpsk_ber.append(float(row["bpsk_ber"]))
                
            if "qpsk_ber_uncoded" in fieldnames:
    ...
    if bpsk_ber:
        ax.plot(snr_vals, bpsk_ber, 'bo-', label='BPSK (моделирование)', markersize=4, linewidth=1.5)
    if qpsk_ber:
        ax.plot(snr_vals, qpsk_ber, 'rs--', label='QPSK (моделирование)', markersize=4, linewidth=1.5)
    ...
    ax.set_title("Сравнение BPSK и QPSK")
    ax.set_yscale("log")
