...
- `--crc <list>` — список CRC через запятую из `3`, `8`, `16`, `24a`
  (default `3,8,16`). `3` — CRC-3/GSM (`x³+x+1`), `8` — CRC-8/CCITT
  (`x⁸+x²+x+1`), `16` — CCITT/3GPP CRC-16 (`x¹⁶+x¹²+x⁵+1`), `24a` — 3GPP
  TS 38.212 CRC-24A. Чем меньше `r`, тем выше `P_undetected` и тем легче его
  измерить напрямую: CRC-3 (потолок `2⁻³ = 0.125`) гладок уже на ~10⁴ блоков,
  CRC-16 (`2⁻¹⁶`) требует ~10⁶–10⁷. CRC-24A (`2⁻²⁴`) прямым счётом не измерим
  (нужно ~10⁹ блоков/точку) и в дефолт не входит.
